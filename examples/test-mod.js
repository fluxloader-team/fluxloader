...
function logASTNode(msg, node, limit = -1) {
	...
	if (limit != -1)
		displayString = displayString.length > limit ? displayString.substring(0, limit) + ` ... (${displayString.length - limit} more)` : displayString;
...
exports.patches = [
	{
		...
		action: (root) => {
			...
			root.find("function", { params: ["r", "i", "s"] }, (node) => {
				...
				if (
					node.doesMatchRaw(node.astNode._parent, {
...
exports.config = [
	{
		...
		default: "stringPropValue",
		type: "string",
	},
	{
		name: "propertyRegex",
		label: "Regex Property",
		default: "regexPropValue",
		type: "regex",
		regex: "^[a-zA-Z0-9]*$",
	},
	{
